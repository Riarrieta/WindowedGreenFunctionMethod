      module GF_num_exb ! Numerical calculation of the half-space
                         ! electromagnetic Green's tensor
                         ! by discretization of the Sommerfeld integrals
                         !
                         ! This program uses an external subroutine
                         !    to compute the Bessel functions
c-----------------------------------------------------------------------
c               Authors:
...
c------------------------------------------------------------------
      ...
      real*8 :: lambda_min = 400d0         ! wavelength [nm]
      real*8 :: lambda_max = 400d0         ! wavelength [nm]
      real*8 :: z1 = 40d0                  ! height of source above substrate [nm]
      real*8 :: z2 = 40d0                  ! height of observation point above substrate [nm]
      real*8 :: rho = 40d0                 ! horizontal distance between source and observation point [nm]
      real*8 :: eps_2_r = 2.5d0            ! real part of substrate relative permittivity
      real*8 :: eps_2_i = 0.01d0           ! imaginary part of substrate relative permittivity >= 0
      real*8 :: eps_1 = 1d0                ! relative permittivity upper half-space
      complex*16 GTxx,GTyy,GTzz,GTxz,GTzx  ! total Green's tensor * k1^{-3}

      complex*16 GFxx,GFyy,GFzz,GFxz,GFzx  ! homogeneous Green's tensor
      complex*16 GRxx,GRyy,GRzz,GRxz,GRzx  ! reflected Green's tensor

      integer*4, private :: key = 1                 ! 0 = Drude model, 1 = constant permittivity
      character*4, private :: output_prec = "D"     ! "D" = double precision, "S" = single precision
      character*4, private :: lambda_scale = "LIN"  ! "LIN" = lambda linear scale, "LOG" = lambda logarithm scale
      integer*4, private :: nl = 1                  ! number of wavelength to compute Green's function
      real*8, private :: lambda_p = 136.1d0         ! wavelength of plasma frequency for Drude model [nm]
      real*8, private :: gd = 0.0019d0              ! Drude model parameter
      real*8, private :: eps0 = 5d0                 ! Drude model parameter
      integer*8, private :: N1 = 10000              ! number of discretization points in numerical integration
      integer*8, private :: N2 = 10000              ! number of discretization points in numerical integration
      integer*8, private :: N3 = 10000              ! number of discretization points in numerical integration
      real*8, private :: y_max = 1d4                ! parameter
      real*8, private :: div = 10d0                 ! parameter
      ...
      complex*32 eps1
      complex*16 cu, cz, eps, eps_2, kzeps
      complex*16 R, Rpr
...
c
      !open(unit=70, file='GF_LIB\GF.par', status='old', err=101)
      !read(70,*) hdummy
      !read(70,*) hdummy
...
*         write(*,*) 'Correct the error in GF_num_exb.f and try again'
*         return
*      end if
...
*      write(*,*) 'nl:', nl
      if(lambda_min.lt.rz) then
         ...
         write(*,*) 'Correct the error in gf_mod.f90 and try again'
         return
