...
*
*  work   (workspace) real*8/complex*16 array, length lwork
...
*
         if (ihist.ne.0) then
	        write(*,'(10x,A39)') 'CONVERGENCE HISTORY FOR GMRES'
     ...
     &                              icntl(7)
           write(ihist,'(A27,E10.2)') 'Tolerance for convergence: ', 
     &                                cntl(1) 
* 
           write(ihist,'(A53)') 
     ...
     &       '    the residual is normalised by ||b||'
             ...
             write(ihist,'(A8,E10.2,$)') '        ', sA
             write(ihist,'(A11,E10.2)') ' * ||x|| + ', sb
           endif
           sPA      = cntl(4)
           sPb      = cntl(5)
     ...
     &            'preconditioned system (P1)A(P2)y = (P1)b:'
           if ((sPA.eq.DZRO).and.(sPb.eq.DZRO)) then
             ...
             write(ihist,'(A8,E10.2,A3,$)') '        ', sPA, ' * '
             write(ihist,'(A12,E10.2)') '||(P2)y|| + ', sPb
           endif
*
...
*           
           write(*,'(A11,$)') ' Iteration '
           write(*,'(A27)') '  Arnoldi b.e.    True b.e.'
         endif
*
       endif
...
*
        subroutine zgmres(n,m,b,x,H,w,r0,V,yCurrent,xCurrent,rotSin,
...
*
*  H        (workspace)  real*8/complex*16
*           Hessenberg matrix built within dgmres
*
*  w        (workspace)  real*8/complex*16
*           Vector used as temporary storage
*
*  r0       (workspace)  real*8/complex*16
*           Vector used as temporary storage
*
*  V        (workspace)  real*8/complex*16
*           Basis computed by the Arnoldi's procedure.
*  
*  yCurrent (workspace) real*8/complex*16
*           solution of the current LS
*
*  xCurrent (workspace) real*8/complex*16
*           current iterate
*
*  rotSin   (workspace) real*8/complex*16
...
* ----------------------
       integer  irc(*)
       complex*16 x(*), H(m+1,*), w(*), r0(*), V(n,*), yCurrent(*)
       complex*16 xCurrent(*), rotSin(*)
!       real*8    rotCos(*)
       complex*16    rotCos(*)
*
* Local variables
* ---------------
       ...
       complex*16 dVi, temp, aux
       complex*16 auxHjj, auxHjp1j
*
...
* Intrinsic function
* ------------------
       intrinsic zabs, dsqrt, dreal, dconjg
*
*       Executable statements
*
* setup some pointers on the workspace
...
* Apply previous Givens rotations to the new column of H
       do j=1,jH-1
         call zrot(1, H(j,jH), 1, H(j+1,jH), 1, rotCos(j), rotSin(j))
       enddo
       auxHjj = H(jH,jH)
       auxHjp1j= H(jH+1,jH)
       call zrotg(auxHjj, auxHjp1j, rotCos(jH),rotSin(jH))
* Apply current rotation to the rhs of the least squares problem
       call zrot(1, H(jH,m+1), 1, H(jH+1,m+1), 1, rotCos(jH),
     &            rotSin(jH))
*
...
* rotation to the last entry of the collumn
       call zrot(1, H(jH,jH), 1, H(jH+1,jH), 1, rotCos(jH), rotSin(jH))
       H(jH+1,jH) = ZERO
*
* Get the Least square residual
*
       dnormres = zabs(H(jH+1,m+1))
       if (sPa.ne.DZRO) then
*
...
* Save the backward error on a file if convergence history requested
         if (ihist.ne.0) then
           write(ihist,'(I5,11x,E10.2,$)') iterOut*m+jH,bea
           write(ihist,'(7x,E10.2)') be
         endif
*
       endif
*
...
!               
               write(*,*)
               write(*,'(A20)') 'Convergence achieved'
             endif
           ...
           if (ihist.ne.0) then
             ...
             write(ihist,'(A10,E10.2)') 'system:   ', rinfo(1)
             write(ihist,'(A29,$)') 'B.E. on the unpreconditioned '
             write(ihist,'(A8,E10.2)') 'system: ', rinfo(2)
             write(*,'(A27,$)') 'B.E. on the preconditioned '
             write(*,'(A10,E10.2)') 'system:   ', rinfo(1)
             write(*,'(A29,$)') 'B.E. on the unpreconditioned '
             write(*,'(A8,E10.2)') 'system: ', rinfo(2)             
           endif
           info(2) = iterOut*m+jH
           if (ihist.ne.0) then
...
* Save the backward error on a file if convergence history requested
         if (ihist.ne.0) then
           ...
           write(ihist,'(9x,A2)') '--'           
*           
           write(*,'(I5,11x,E10.2,$)') iterOut*m+jH,bea
           write(*,'(9x,A2)') '--' 
         endif
*
       endif  
