...
program MagDip_test
    ...
    k = 3d0 * PI                    ! free-space wavenumber [1/m]
    ...
    !call genEFIEDuffyMat(Z3, msh, k, nNS, nNS2)
...
!        Hinc = Hinc / (w * mu0)        ! adjust constants
        ...
        ! test RHS
        V1 = k**2 / (IU *w * mu0) * matmul(Ztest, Mvec)
        ...
        ! Solve
        call linsolve(I1, -0.5d0*Emat+Zmfie, V1, msh%nbEdg) ! test
...
!        Eexact = -Eexact         ! adjust constants
    ...
    if (id==0) then
        ...
        Es1(:,1) = matmul(Epot(:,:,1), I1) - matmul(Hpot(:,:,1), Mvec)
        Es1(:,2) = matmul(Epot(:,:,2), I1) - matmul(Hpot(:,:,2), Mvec)
        Es1(:,3) = matmul(Epot(:,:,3), I1) - matmul(Hpot(:,:,3), Mvec)
        Es1 = Es1 - Eexact
