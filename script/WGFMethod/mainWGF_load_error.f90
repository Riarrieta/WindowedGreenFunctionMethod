...
program mainWGF_load_error
    ...
    character(len=100), parameter :: file_msh = 'meshes/plane_mesh/plane_mesh_a20_h03.msh'
    character(len=100), parameter :: file_currents = 'results/data.txt'
    character(len=100), parameter :: file_evl_msh = 'meshes/eval_msh/disk_evl_r10_z2.msh'
    real(8) :: win_radius                       ! Window radius
    real(8), parameter :: win_cparam = 0.7d0    ! Window 'c' parameter

    ! Dipole/planewave parameters
    real(8), parameter :: pol(3) = [0d0, 0d0, 1d0]    ! dipole polarization vector
    real(8), parameter :: src(3) = [0d0, 0d0, 3.1d0]    ! location of dipole source
    ...
    if (id == 0) print *, "Make sure dipole parameters match"
