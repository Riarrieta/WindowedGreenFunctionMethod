...
c------------------------------------------------------------------
      ...
      real*8 eps, eps_1, eps_2_r, eps_2, da, db
...
c
      ...
      read(70,*, err=101) key
      if(key .eq. 0) then
         write(*,*) 'GF_sde_tnds ignores the KEY=0 option'
         write(*,*) 'Even though you have given me KEY=0,'
         write(*,*) '   ... I will set KEY=1 and assume'
         write(*,*) '   ... a nondispersive transparent substrate'
         key = 1
      end if
      if (key .eq. 1) then
         write(*,*) 'Assuming that the substrate has a'
         write(*,*) '   ... wavelength-independent, REAL permittivity'
         write(*,*) '   ... eps_2 = eps_2_r + i*0'
         write(*,*) '   ... where eps_2_r is given in GF.par'
         write(*,*) 'If you dont like this, use GF_sde_tdss,'
         write(*,*) '   ... set KEY=2,'
         write(*,*) '   ... and program your own formula for eps_2_r'
         write(*,*) '   ... But make sure that eps_2 is REAL'
      else if (key .eq. 2) then
         write(*,*) 'KEY=2 is not allowed in GF_sde_tnds'
         write(*,*) 'If you wish to program your own'
         write(*,*) '   ... wavelength-dependent eps_2_r,'
         write(*,*) '   ... use GF_sde_tdss'
         write(*,*) 'Exiting'
         stop
      else 
 ...
 5    continue
     ...
     $                                     sngl(lambda_max), '  [nm]'
      ...
      read(70,*) eps_2_r, rdummy
      if(eps_2_r .lt. rz) then
         write(*,*) 'Error. Incorrect parameter: eps_2_r<0; 
     $                                           eps_2_r=', eps_2_r
         write(*,*) 'GF_sde_tnds only allows positive eps_2_r'
         write(*,*) 'Correct the error in GF.par and try again'
         stop
      end if
...
c     higher orders (only imaginary parts of Gab):
...
c     higher orders in h-expansion in refl part:
     ...
     $       (bi4*ep2*Lam-pi9/v_p)+rhoh4*(pi10/v_p+bi4*ep3*Lam))/bi1
      Kzz7 = cu*s4*(8.*zh4*(pi11/v_p-bi4*ep2*Lam)/3.-8.*zh2*rhoh2*
     $       (bi4*ep3*Lam+pi12/v_p)+rhoh4*(pi13/v_p-bi4*ep4*Lam))/bi2
